...
| 7 | 领域模型/实体 | Model/Entity | 领域层 | `domain/model` | 核心业务对象，具有唯一标识和生命周期，封装业务规则和行为 |
| 8 | 领域仓储 | Repository | 领域层 | `domain/repository` | 定义领域对象的持久化接口，屏蔽底层数据访问细节，只定义契约，具体实现在基础设施层 |
| 9 | 领域服务 | Domain Service | 领域层 | `domain/service` | 封装不适合放在单个实体中的业务逻辑，处理跨实体的复杂业务规则 |
| 10 | 领域事件 | Domain Event | 领域层 | `domain/event` | 记录领域内的重要业务事件，用于实现最终一致性、解耦子系统、发布订阅模式 |
| 11 | 值对象 | Value Object | 领域层 | `domain/value` | 描述特征但无身份标识的对象，通过属性值判断相等性，如 Money、Address 等 |
| 12 | 领域异常 | Domain Exception | 领域层 | `domain/exception` | 定义业务规则违反时的自定义异常，用于表达领域特定的错误情况 |
